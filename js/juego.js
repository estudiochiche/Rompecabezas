...
const instruccion1 = "Tu misión es ordenar las diferentes piezas del rompecabezas hasta obtener la misma imagen que hay en el objetivo.";
const instruccion2 = "Para mover las piezas utilizá las flechas del teclado y podrás mover tu pieza hacia donde está el logo de los avengers.";

let instrucciones = [instruccion1, instruccion2];
...
function chequearSiGano() {
  let numeroComparador = 1;

  for (let i = 0; i < grilla.length; i++) {
    for (let j = 0; j < grilla[i].length; j++) {
      
        if(grilla[i][j] != numeroComparador){
          return false
        } 
        numeroComparador++;
    }
...
function mostrarCartelGanador() {
  alert("Felicitaciones has completado correctamente el rompecabezas");
};
...
function intercambiarPosicionesGrilla(filaPos1, columnaPos1, filaPos2, columnaPos2) {
  let nuevaPosicion = grilla[filaPos1][columnaPos1];
